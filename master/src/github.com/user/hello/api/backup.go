...
//This File contains job dependency
/*
...
func jobRunner(jobQueue chan string, nodeMsgChan chan<- NodeMsg, gMsgChan chan<- GMsg) {
	...
	for {
		...
		notRunnableJobQueue := make(chan DockerJob, 1000)

		for jobName := range jobQueue {
		...
		for notRunnedJob := range notRunnableJobQueue {
			jobQueue <- notRunnedJob.JobName
		}
		//sendJob
